...
# ---------------------------------------------------------------------------
# Two modifications to the released implementation. Both default to ON; set the
# environment variable to "0" to reproduce the original behaviour exactly.
#
#   FOLD_NORMALISATION_INTO_COLORS
#       Moves the PCA de-normalisation constant out of the renderer output and
#       into the optimised colour parameters. The initial render is unchanged;
#       the gradient w.r.t. the colours is no longer scaled by ~1/23.
#       Applied in update_gaussian(). Effect: +5.3 to +15.3 dB at 1000
#       iterations across seven image domains.
#
#   NORMALISE_LUMINANCE_BEFORE_PROJECTION
#       Rescales the input image's Y channel to the basis training set's own
#       mean and standard deviation BEFORE the PCA projection that produces the
#       initial colours, then optimises against the true (unmodified) image.
#       Only affects where optimisation starts. Applied in update_gaussian().
#       Effect: recovers the failure that appears when the input's mean
#       luminance is far from the basis's (>= 2 sd on the first principal
#       component); inert on unmodified input, HARMFUL on in-domain material.
# ---------------------------------------------------------------------------
FOLD_NORMALISATION_INTO_COLORS = os.environ.get("EIGENGS_FOLD_SCALE", "1") != "0"
NORMALISE_LUMINANCE_BEFORE_PROJECTION = os.environ.get("EIGENGS_RELUM", "0") != "0"

# Basis luminance statistics, measured on the images the PCA was fitted to
# (ImageNet, Y channel in [0, 1]). Override for a different basis.
BASIS_Y_MEAN = float(os.environ.get("EIGENGS_BASIS_Y_MEAN", "0.4402"))
BASIS_Y_STD = float(os.environ.get("EIGENGS_BASIS_Y_STD", "0.2072"))
...
class GaussianTrainer:
    ...
    def update_gaussian(self):
        ...
        self.gaussian_model.cur_freq = "all"
        proj_image = self.gt_image
        if NORMALISE_LUMINANCE_BEFORE_PROJECTION:
            # ---- luminance normalisation (see README) ----------------------
            # The mean image Psi_0 is a non-learnable buffer, so a mismatch
            # between the input's brightness and the basis's cannot be absorbed
            # there -- it is pushed onto the coefficients, and because Psi_0 is
            # nearly parallel to psi_1 it lands almost entirely on the FIRST
            # component (measured: 91.7% of the displacement energy at k=0.25).
            # Rescaling Y to the basis's own statistics before projecting moves
            # the starting point back inside the basis's distribution.
            # NOTE: this LOWERS the initial PSNR (18.9 -> 14.8 dB at k=0.25) and
            # wins on trajectory, not on initial agreement.
            proj_image = self.gt_image.clone()
            y = proj_image[0]
            proj_image[0] = (((y - y.mean()) / (y.std() + 1e-8)) * BASIS_Y_STD
                             + BASIS_Y_MEAN)
            proj_image = proj_image.clamp(0.0, 1.0)
        img_arr = proj_image.detach().cpu().numpy()
        ...
        with torch.no_grad():
            ...
            if FOLD_NORMALISATION_INTO_COLORS:
                # ---- gradient-path fix (see README) ------------------------
                # `scale_factor` = max - min over the whole PCA component
                # matrix; measured 0.0407 / 0.0457 / 0.0434 for Y / Cb / Cr,
                # i.e. 1/scale_factor = 23.2 +- 1.3. It is applied to the
                # RENDERER OUTPUT in GaussianBasis._forward_colors:
                #     out_img *= scale_factor ; out_img += shift_factor
                # so it also multiplies d(loss)/d(_colors). Measured at
                # iteration 1: |grad| = 1.06e-07 with the constant in the
                # output path, 2.58e-06 with it folded in here -- a factor
                # 24.2, matching 1/scale_factor.
                #
                # Rasterisation is linear in the colours, so folding the
                # constant into _colors and leaving the output path as the
                # identity gives the SAME image:
                #     gamma * sum_n C_n g_n + beta
                #         = sum_n (gamma C_n) g_n + beta
                # The initial render is unchanged; only the gradient w.r.t.
                # the optimised colours is no longer scaled by gamma.
                self.gaussian_model._colors.mul_(
                    self.gaussian_model.scale_factor.view(1, 3))
                self.gaussian_model.image_mean = (
                    self.gaussian_model.image_mean
                    + self.gaussian_model.shift_factor.view(3, 1))
                self.gaussian_model.scale_factor = torch.ones(
                    3, device=self.gaussian_model.device)
                self.gaussian_model.shift_factor = torch.zeros(
                    3, device=self.gaussian_model.device)
                mean_tensor = self.gaussian_model.image_mean

...
class GaussianTrainer:
    def __init__(
        ...
        if image_path is not None:
            ...
            model_dir = Path(f"./models/recons-ycbcr-single/{self.dataset_path.name}/{self.image_name}")
        else: 
        ...
        self.gaussian_model = GaussianBasis(
            ...
            gt_arrs=self.gt_arrs).to(self.device)
...
def get_report(args, results):
    model_dir = Path(f"./models/recons-ycbcr-single/{Path(args.dataset).name}")
